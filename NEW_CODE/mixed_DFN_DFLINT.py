...
# le prime NINT variabili sono discrete le successive N-NINT sono continue
# la i-esima variabile discreta assume valori
...
def box_DFN_DFL(alg, M, J, V, CACHE, max_fun, outlev):
    ...
    if(nint <= 1):
        print('\n\nERROR: number of integer variables must be > 1')
...
#        print(eps.shape)
    ...
    if outlev > 0:
        ...
        print('\n', file = log)
        print('   |  iter |    nf | cache |        fpen     |        f_ref    |         viol    |    max_alpha    |  ndir |', file = log)
...
#########################################################
#########################################################
#       esplorare le variabili continue
#########################################################
#########################################################
        if ncont > 0:
            CS.ni = iter
            nf, cambio_eps = CS.step(alg,x[ncont:],f,eps,V,CACHE,nf)
            iter = CS.ni
...
#########################################################
            ...
            if outlev >= 2:
                print('discrete_search along d',f_ref,max(W),W)
...
#########################################################
            if outlev >= 2:
                print('discrete_search returned alpha = ',alpha)

            if alpha <= 0:
                d = -d

                if outlev >= 2:
                    print('discrete_search along -d')
            ...
            if outlev > 0:
                ...
                print(print_format %(iter, nf, CACHE.hits, fy, f_ref, np.sum(np.where(g<0,0,g)), max(alpha_tilde), idir,ndir), file = log)
                print(print_format %(iter, nf, CACHE.hits, fy, f_ref, np.sum(np.where(g<0,0,g)), max(alpha_tilde), idir,ndir))
                if outlev >= 2 and iter >= 60:
                    print('Paused')
                    input()
        ...
        if (np.linalg.norm(y-x) <= 1e-14) and (((alg == 'DFN_DFL') and (ndir >=5000)) or (alg == 'DFL' and (max(alpha_tilde) == 1) and (old_maxalpha == 1) and (CS.alfa_max <= tol))):
        ...
        if outlev > 0:
            ...
            print(print_format %(iter, nf, CACHE.hits, f, f_ref, np.sum(np.where(g<0,0,g)), max(alpha_tilde), ndir,ndir), file = log)
            print(print_format %(iter, nf, CACHE.hits, f, f_ref, np.sum(np.where(g<0,0,g)), max(alpha_tilde), ndir,ndir))
...
def functpen(xint,eps,J,V,CACHE):
    ...
    if V.m[J] > 0:
        ...
        #print('FUNCTPEN:   g1 = ',np.maximum(0.0,np.max(gloc)),J)
        #print('FUNCTPEN:    g = ',gloc)
        #print('FUNCTPEN: viol = ',np.sum(np.where(gloc<0,0,gloc)))
        #input()
...
def nm_discrete_linesearch(y,d,alpha_tilde,lb,ub,f_ref,eps,xi,m,ncont,nf,J,V,CACHE,outlev):
    ...
    if outlev >= 2:
        print('nm_discrete_search: alpha_init = ',alpha_init)
    ...
    if (alpha_init > 0):
        ...
        if (mn<=1e-16):
            ...
            CACHE.hits += 1
            if outlev >= 2:
                print('nm_discrete_search: f_trial computed by cache')
        else:
            f_trial = functpen(y_trial,eps,J,V,CACHE)
            nf += 1
            if outlev >= 2:
                print('nm_discrete_search: f_trial computed by functpen',f_trial)
                print('                  : fob  = ',CACHE.xf[CACHE.nnf-1,n])
                gval = CACHE.xf[CACHE.nnf-1,n+1:]
                print('                  : viol = ',np.sum(np.where(gval<0,0,gval)))
                print('                  : gval = ',gval)
                print('                  : eps  = ',eps)
    else:
        f_trial = np.inf
    ...
    if outlev >= 2:
        print('nm_discrete_search: ftrial = ',f_trial)
        print('                  : f_ref  = ',f_ref)
    ...
    if (alpha_init > 0) and (f_trial <= f_ref - xi):
        ...
        if outlev >= 2:
            print('nm_discrete_search: f_trial <= f_ref - xi')
        alpha=alpha_init
        x = y_trial
        f = f_trial
        ...
        while (alpha<alpha_bar) and (f_trial <= f_ref - xi):
            ...
            if outlev >= 2:
                print('nm_discrete_search: f_trial <= fref - xi (while loop)')
